...
import { registerOptionsEmail, registerOptionsPassword, registerOptionsConfirmPassword } from '../../constants/validate-register.options';
...
export const RegisterForm = ({ className, ...props }: RegisterFormProps) => {
	const { handleSubmit, register: registerField, watch, formState: { errors } } = useForm<IRegisterForm>();
	...
	return (
		<form className={cn(className, common_styles.form)} onSubmit={handleSubmit(onSubmit)} {...props}>
			...
			<span className={common_styles.footer}>Уже зарегистрированны? <Link to={'/auth/login'} >Войдите!</Link></span>
		</form>
	);
};